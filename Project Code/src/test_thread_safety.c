...
/*
 * Thread-safety verification uses three complementary strategies:
 *
...
int run_thread_safety_tests(void) {
    int failed = 0;
    ...
    printf("  mat_mat_mult_tiled:               %s\n",
           failed == 0 ? "PASS" : "FAIL (see above)");

    int prev = failed;
    /* The three tests above add up to 3 possible failures. */
    int det_fails = failed;
    (void)det_fails;
    ...
    printf("  stress standard:    %s\n", failed == prev ? "PASS" : "FAIL");
    /* Just report overall */
    int tests_run = 9;  /* 3 categories × 3 functions */
    printf("\nThread safety: %d tests, %d failed\n", tests_run, failed);
    return failed;
}
